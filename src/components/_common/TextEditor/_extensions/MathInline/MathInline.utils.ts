...
export const deleteMath = (state: any, dispatch: any, view: any) => {
  const {
    tr,
  ...
  const rborder =
    from.parent.type !== state.schema.nodes.mathInline &&
    from.doc.resolve(from.pos - 1).parent.type === state.schema.nodes.mathInline
  if (rborder) {
    const mathNode = from.doc.resolve(from.pos - 1).parent
    const startPos = from.pos
    tr.replaceRangeWith(
      startPos - mathNode.nodeSize,
      startPos,
